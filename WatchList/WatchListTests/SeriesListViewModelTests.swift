...
class SeriesListViewModelTests: XCTestCase {
    ...
    func testCellForItemAtIndexPathForSection() {
        ...
        /// set the SeriesListViewModel
        let seriesListViewModel = SeriesListViewModel(withSeriesList: programOptionsViewModel.provideDataForSeries())
        /// data source for series view collection cell
        let dataSource = seriesListViewModel.cellForItemAtIndexPathForSection(row: 0)
    ...
    func testNumberOfItemsInSection() {
        ...
        let seriesListViewModel = SeriesListViewModel(withSeriesList: programOptionsViewModel.provideDataForSeries())
        /// data source for series view collection cell
        let assetCount = seriesListViewModel.numberOfItemsInSection()
        XCTAssertEqual(assetCount, 45)
        /// Expectation fullfil
        expectation.fulfill()
        self.waitForExpectations(timeout: 5.0)
    }
